            ...
            setStore(
              produce((draft) => {
                ...
                for (const message of visible) {
                  ...
                  const parts = !fetched
                    ? currentParts
                    : optimisticMessages.has(message.id) && fetched.parts.length === 0
                      ? currentParts
                      : fetched.parts.flatMap((part) => {
                  ...
                  if (fetched && optimisticMessages.has(message.id) && fetched.parts.length > 0) {
                    optimisticMessages.delete(message.id)
                  }

                  const mergedParts = [...parts]
                  mergedParts.push(
                    ...currentParts.filter(
                      (part) => tracker.parts.has(part.id) && !mergedParts.some((item) => item.id === part.id),
                    ),
                  )
                  draft.part[message.id] = mergedParts
                }
